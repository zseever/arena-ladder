...
export default function LeaderPage() {
    ...
    async function fetchData(brkt, rgn) {
      const url = `https:/${ rgn === '1' ? 'us' : 'eu'}.api.blizzard.com/data/wow/pvp-region/${rgn}/pvp-season/8/pvp-leaderboard/${brkt}?namespace=dynamic-classic-${rgn === '1' ? 'us' : 'eu'}&locale=en_US&access_token=USCY1VU77hm4d737wH7EJsrBGua47n5nHm`
            ...
            <table className="ranking-table">
                ...
                { data.length && data.slice(0,50).map((char,idx) => 
                    <tr className={idx % 2 === 0 ? 'even-row' : 'odd-row'} key={char.character.name}>
                        <td><Link to='/hello' state={{ name: char.character.name, server: char.character.realm.slug, region: region}}>{char.character.name}</Link></td>
